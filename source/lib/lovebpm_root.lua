...
function lovebpm.detectBPM(filename, opts)
  ...
  if type(data) == "string" then
    data = snd.sampleplayer.new(data)
  else
    data = filename
  end
...
function Track:load(filename)
  ...
  print("lovebpm.lua [110] filename: " .. filename)
  self.source = playdate.sound.fileplayer.new(filename)
  Track:setLooping(self.looping)
  self:setVolume(self.volume)
  Track:setPitch(self.pitch)
  print("lovebpm.lua [115] Track")
  printTable((Track))
  self.totalTime = self.source:getLength("seconds")
  --printTable(self)
  Track:stop()
  return self
end
...
function Track:setLooping(loop)
  self.looping = loop
  -- if self.source then
  --   print("self.source: " .. tostring(self.source))
...
function Track:stop()
  ...
  self.lastUpdateTime = nil
  self.lastSourceTime = 0
  print("lovebpm.lua [207] self.source")
  print(self.source)
  if self.source then
    --self.source:stop()
  end
