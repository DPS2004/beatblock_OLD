  ...
  if (obj.hb - cs.cbeat) <= 0 then
    if obj.hold == false then
      ...
      if helpers.angdistance(obj.angle,cs.p.angle) <= cs.p.paddle_size / 2 then 
        ...
        obj.angle = helpers.interpolate(obj.endangle,obj.angle2,((obj.hb - cs.cbeat)*-1)/obj.duration,obj.easeshape,obj.easetype)
...
function obj.draw()
  ...
  if not obj.slice then
    ...
    if not obj.hold then
      ...
      helpers.drawhold(obj.ox, obj.oy, obj.x, obj.y, obj.x2, obj.y2, obj.angle, obj.angle2, obj.segments, obj.spr3, obj.easeshape, obj.easetype)
    end
  else
