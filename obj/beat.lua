...
function obj.draw()
  ...
  if not obj.slice then
    ...
    if obj.inverse then
      p1 = helpers.rotate(-(obj.hb - cs.cbeat)*cs.level.speed*obj.smult,obj.angle,obj.ox,obj.oy)
    else
      p1 = helpers.rotate((obj.hb - cs.cbeat)*cs.level.speed*obj.smult,obj.angle,obj.ox,obj.oy)
    end
    obj.x = p1[1]
    ...
    helpers.color(1)
    love.graphics.circle("fill",obj.x,obj.y,4+cs.extend/2+(math.sin(cs.p.bobi))/2)
    helpers.color(2)
    love.graphics.circle("line",obj.x,obj.y,4+cs.extend/2+(math.sin(cs.p.bobi))/2)
